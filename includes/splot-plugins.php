	...
	function __construct() {
		...
		$this->plugins = array(			
			array(
				...
				'message' => sprintf(
					esc_html__( 'The %1$s allows you to create a menu of social icon links. If you have the version 4 plugin, please de-activate and delete.', 'highlights' ),
					'<strong>' . esc_html__( 'Font Awesome 5 for Menus plugin', 'highlights' ) . '</strong>' ),
			),
	...
	function admin_notices() {
		...
		foreach ( $this->plugins as $key => $plugin ) {
			...
			if ( 'to-activate' === $plugin['status'] ) {
				...
				$notice .= sprintf(
					esc_html__( ' To do this, activate %1$s. %2$s', 'highlights' ),
					esc_html( $plugin['name'] ),
					( $activate_url ) ? '<a href="' . $activate_url . '">' . esc_html__( 'Activate Now', 'highlights' ) . '</a>' : ''
				);
			}
			...
			if ( 'to-install' === $plugin['status'] ) {
				...
				if ($install_url) {
					...
					$notice .= sprintf(
						esc_html__( ' To do this, install %1$s. %2$s', 'highlights' ),
						esc_html( $plugin['name'] ),
						( $install_url ) ? '<a href="' . $install_url . '">' . esc_html__( 'Install Now', 'highlights' ) . '</a>' : ''
					);
				} elseif ( $plugin['download']  ) {
					$notice .= sprintf(
						esc_html__( ' To use this plugin, download %1$s. %2$s and manually install.', 'highlights' ),
						esc_html( $plugin['name'] ),
						( $plugin['download'] ) ? '<a href="' . $plugin['download'] . '" target="_blank">' . esc_html__( 'Download source', 'highlights' ) . '</a>' : ''
					);	
				}
			}
...
function splot_dismiss_admin_notice() {
	...
	if ( ! isset( $_POST['splot_admin_nonce'] ) || ! wp_verify_nonce( $_POST['splot_admin_nonce'], 'splot_admin_nonce' ) ) {
		wp_die( __( 'Your request failed permission check.', 'highlights' ) );
	}
	...
	wp_send_json( array(
		'status' => 'success',
		'message' => __( 'Your request was processed. See ya!', 'highlights' )
	) );
}
